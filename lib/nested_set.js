...
var NestedSetPlugin = function(schema, options) {
  ...
  schema.add({ rootId: {type: Schema.ObjectId} });

  schema.index({parentId: 1});
  schema.index({lft: 1, rgt: 1});
  schema.index({rgt: 1});
  schema.index({rootId: 1});
  
  schema.pre('save', function(next) {
    var self = this;

    // Tree has already been built, and all values assigned to this instance. Re-assigning values can cause mismatched lft and rgt values. 
  ...
  schema.static('rebuildTree', function(parent, left, rootId, callback) {
    var self = this;
    parent.lft = left;
    parent.rgt = left + 1;
    ...
    self.find(params, function(err, children) {
      ...
      if (children.length > 0) {
        async.forEachSeries(children, function(item, cb) {
          self.rebuildTree(item, parent.rgt, rootId, function() {
  ...
  schema.method('isDescendantOf', function(other) {
    var self = this;
    return other.lft < self.lft && self.lft < other.rgt && self.rootId === other.rootId;
  });

  // Returns true if other is an ancestor of self
  schema.method('isAncestorOf', function(other) {
    var self = this;
    return self.lft < other.lft && other.lft < self.rgt && self.rootId === other.rootId;
  });
  ...
  schema.method('selfAndAncestors', function(callback) {
    var self = this;
    self.constructor.where('lft').lte(self.lft).where('rgt').gte(self.rgt).where('rootId').equals(self.rootId).exec(callback);
  });
  ...
  schema.method('selfAndDescendants', function(callback) {
    var self = this;
    self.constructor.where('lft').gte(self.lft).where('rgt').lte(self.rgt).where('rootId').equals(self.rootId).exec(callback);
  });
